...
proc_main: BEGIN
    ...
    IF v_role_id IS NULL THEN
        SET p_result_message = '用户不存在或账户已禁用';
        SET p_template_id = 0;
        ROLLBACK;
        LEAVE proc_main;
    END IF;
    ...
    IF NOT v_has_permission THEN
        SET p_result_message = '没有创建报表模板的权限';
        SET p_template_id = 0;
        ROLLBACK;
        LEAVE proc_main;
    END IF;
    ...
    IF EXISTS(SELECT 1 FROM 报表模板 WHERE 报表名称 = p_report_name) THEN
        SET p_result_message = CONCAT('报表名称已存在："', p_report_name, '"');
        SET p_template_id = 0;
        ROLLBACK;
        LEAVE proc_main;
    END IF;
    ...
    SET p_result_message = CONCAT('模板创建成功，编号：', p_template_id, '，等待审核');
